...
try{
    $sql = "UPDATE {$_POST['table']} SET brand = :brand, branch = :branch, address = :address, time = :time, tell = :tell, map = :map, img = :img WHERE id = :id";
    $stmt = $pdo->prepare($sql);
    $params = array(':brand' => $_POST["brand"], ':branch' => $_POST["branch"], ':address' => $_POST["address"], ':time' => $_POST["time"], ':tell' => $_POST["tell"], ':map' => $_POST["map"],':img' => $img, ':id' => $_POST["shop_id"]);
    $stmt->execute($params);
 ...
 <div id="wrapper" class="container w-80">
    <button type="button" class="btn btn-outline-secondary w-100" onclick="location.href='../template/index.php'">TOPへ戻る</button><br>
      <div class="box_shop mt-0">
        ...
        <img src="../img/<?=$_POST['table']?>/<?=$img?>" alt="<?=$_POST['brand']?>" width="400" height="300">
