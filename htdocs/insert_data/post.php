...
$tmp_file = isset($_FILES["pic"]["tmp_name"]) ? $_FILES["pic"]["tmp_name"] : "";
$org_file = isset($_FILES["pic"]["name"])     ? $_FILES["pic"]["name"]     : "";
if( $tmp_file != "" &&
   is_uploaded_file($tmp_file) )
{
 ...
 if( $ext != "" &&
     $ext != $org_file )
 {
   $img = date("Ymd_His.") . mt_rand(1000,9999) . ".$ext";
   $up_file = "../img/".$_POST['item']."/".$img;
...
try{
    if ($_POST['checkbox'] == "name"){
        $sql = "INSERT INTO `{$_POST['item']}` (`name`, `address`, `access`, `time`, `tell`, `img`, `map`) VALUES('{$_POST["name"]}', '{$_POST["address"]}', '{$_POST["access"]}', '{$time}', '{$tell}', '{$img}', '{$_POST["map"]}')";
    } else if ($_POST['checkbox'] == "brand"){
        $sql = "INSERT INTO `{$_POST['item']}` (`brand`, `branch`, `address`, `access`, `time`, `tell`, `img`, `map`) VALUES('{$_POST["brand"]}', '{$_POST["branch"]}', '{$_POST["address"]}', '{$_POST["access"]}', '{$time}', '{$tell}', '{$img}', '{$_POST["map"]}')";
    }
    $pdo -> query($sql);
    ...
    <button type="button" class="btn btn-outline-secondary w-100" onclick="location.href='../template/index.php'">TOPへ戻る</button><br>
      <div class="box_shop mt-0">
        <p id="<?=$_POST['id']?>" class="s pt-0">
          ...
          </svg> <?=$_POST['brand']?>　<?=$_POST['branch']?> 
        </p>
        <img src="<?= $up_file ?>" alt="<?=$_POST['brand']?>" width="400" height="300">
        <div class="map"><iframe src="<?=$_POST['map']?>" width="400" height="300"></iframe></div>
        <table class="table">
          <thead class="thead-dark">
            <tr>
              ...
              <td><?=$_POST['brand']?>　<?=$_POST['branch']?></td>
            </tr>
            <tr>
              <th>
